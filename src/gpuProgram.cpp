...

{
  ...
  if (vsText == NULL || fsText == NULL) { // try the '../shaders/' directory

    const char *prefix = "../shaders/";
    char newFile[1000];

    sprintf( newFile, "%s%s", prefix, vsFile );
    vsText = textFileRead( newFile );

    sprintf( newFile, "%s%s", prefix, fsFile );
    fsText = textFileRead( newFile );
  }

  if (vsText == NULL || fsText == NULL) {
    if (vsText == NULL)
      std::cerr << "Vertext shader " << vsFile << " could not be found" << std::endl;
    if (fsText == NULL)
      std::cerr << "Fragment shader " << fsFile << " could not be found" << std::endl;
    exit(1);
  }
